...
public class TranslateOffsetService {
	...
	public TranslateOffsetService(String endpoint, SecretKey specialKey,
	...
	public Integer execute() throws BallotServerException, RemoteException{
		try {
			...
			TranslateOffsetRequestType request = new TranslateOffsetRequestType();
			...
			TranslateOffsetResponseType response = port.translateOffset(request);
			
			Integer offset = new Integer(new String(CriptoUtils.decipherWithSymKey(
					CriptoUtils.base64decode(response.getOffset()), this.specialKey)));
			
			byte[] computedHash = CriptoUtils.computeDigest(
					offset.toString().getBytes(), this.specialKey.getEncoded());
